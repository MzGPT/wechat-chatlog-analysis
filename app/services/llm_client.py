...
DEFAULT_MODULE_PROMPTS: Dict[str, Dict[str, str]] = {
    "market": {
        "system": "\n".join([
            ...
            "必须提炼趋势、驱动与影响，给出可执行的提醒。不得复述原文、不得罗列机构或时间。",
            "如需引用来源，只能在句尾用 `(来源:#123 #456)` 短标，正文不得出现券商、人名、日期。",
        ]),
        "user": "\n".join([
            "请阅读 JSON 并输出 JSON 对象 {\"markdown\": string}，格式严格如下：",
            "`# 市场观点总览`",
            "`- 核心基调：<一句话描述市场多空平衡/驱动/风险>`",
            "`- 今日关键风险：<1-2 个最需关注的风险，含触发条件>`",
            "`## 宏观政策`",
            "`1. 主题：结论；关键证据；风险/待验证` (共 2-4 条，按重要度降序)",
            "`## 行业板块` (2-4 条，突出热点/拐点/行业对比)",
            "`## 公司基本面` (2-4 条，聚焦业绩/订单/估值变化)",
            "`## 投资策略` (给出资产配置/仓位/风格建议)",
            "`## 市场情绪` (量化描述情绪分布、资金流向或成交特征)`",
            "`## 今日重点提示` (3-4 条行动建议或跟踪事项)`",
            "要求：",
            "- 每条观点最多两行，先结论后依据，用分号连接；强调逻辑链。",
            "- 同一主题只保留一条，合并重复表述；如有分歧，用`(存在分歧：…)` 标注。",
            "- 不得抄写原文或出现券商/机构名称/日期。数据请写成‘同比+/-x%’等结构化描述。",
            "数据：{{messages_data}}",
        ]),
    },
    "meetings": {
    ...
    "newswatch": {
        "system": "你是一名舆情风控分析师，要将多来源新闻归纳为交易层面的日报。必须统计数量、识别主线、给出风险与机会提示。",
        "user": "请阅读 JSON 数据并输出 {\"markdown\": string}，遵循以下结构：\n`# 新闻舆情监测`\n`- 数据概览：今日收录 <总条数> 条，主要来源 <前3家媒体>；类别分布（宏观/行业/个股/...）`\n`## 全局判断`\n`- <总结1：主线/谈判/政策进展 + 影响>`\n`- <总结2：业绩披露/资金面/风险事件 + 影响>`\n`## 主题脉络`\n`1. <主题A>(<条数>条)：核心结论；可能影响；关注要点`\n`2. <主题B>...` (列3-5个，按影响力排序，必要时合并同义主题)\n`## 舆情温度`\n`- 正面/中性/负面条数对比，指出情绪倾向变化`\n`- 监测到的风险点或舆情扩散路径`\n`## 关注动作`\n`- 3-4 条可执行的跟踪/沟通/仓位建议`\n约束：\n- 只能写综合总结，不得粘贴原新闻标题或媒体名；引用来源仅能在句尾用 `(来源:#123 #456)` 简短标注。\n- 同一主题若多条新闻，务必合并，写明“X 条报道集中于…”。\n- 避免无意义的客套语，重点描述趋势、驱动因素、潜在催化与风险。\n数据：{{messages_data}}"
    },
    "socialwatch": {
