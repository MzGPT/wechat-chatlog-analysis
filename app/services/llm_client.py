...
DEFAULT_MODULE_PROMPTS: Dict[str, Dict[str, str]] = {
    ...
    "contacts": {
        "system": "\n".join([
            ...
            "仅基于评分筛选联系人（移除活跃度）；黑名单与评分<40一律剔除。",
        ...
        "newswatch": {
        "system": "你是舆情风控分析师。请将多来源新闻/快讯整合为面向投研/交易的简报。- 通览全文，合并相近主题；用概括性语言总结，不复制标题；- 可在句尾用 #(id) 标注来源；输出结构清晰、可执行。",
        "user": """
请输出 {\"markdown\": string}：
# 新闻舆情监测
- 总体基调：<一句话概括主线/风险/催化>
